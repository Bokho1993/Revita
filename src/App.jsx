...
var KIDS_HABITS = [
  ...
  { id: "daily_challenge", cat: "mind", label: "Thử thách mỗi ngày", emoji: "🎯", xp: 5, desc_bong: "Ba mẹ giao thử thách", desc_beo: "Ba mẹ giao thử thách" },
];
...
var FINANCE_EXPENSE_CATS = [
  { id: "food", name: "Ăn uống", icon: "🍜", color: "#FF6B6B" },
  { id: "living", name: "Sinh hoạt", icon: "🏠", color: "#3498DB" },
  { id: "education", name: "Giáo dục", icon: "📚", color: "#F39C12" },
  { id: "health", name: "Sức khỏe", icon: "💊", color: "#27AE60" },
  { id: "transport", name: "Di chuyển", icon: "🚗", color: "#9B59B6" },
  { id: "entertainment", name: "Giải trí", icon: "🎮", color: "#E67E22" },
  { id: "shopping", name: "Mua sắm", icon: "🛒", color: "#1ABC9C" },
  { id: "kids", name: "Con cái", icon: "👶", color: "#FF69B4" },
  { id: "other_exp", name: "Khác", icon: "📦", color: "#95A5A6" },
];

var FINANCE_INCOME_CATS = [
  { id: "salary", name: "Lương", icon: "💼", color: "#27AE60" },
  { id: "investment", name: "Đầu tư", icon: "📈", color: "#3498DB" },
  { id: "bonus", name: "Thưởng", icon: "🎁", color: "#F39C12" },
  { id: "other_inc", name: "Thu nhập khác", icon: "💵", color: "#9B59B6" },
];
...
function formatVND(n){if(n>=1000000000)return(n/1000000000).toFixed(1)+"tỷ";if(n>=1000000)return(n/1000000).toFixed(1)+"tr";if(n>=1000)return Math.round(n/1000)+"k";return n+"";}
function getDefault(){return{totalXp:0,gems:10,streak:0,maxStreak:0,totalDays:0,perfectDays:0,sleepStreak:0,readStreak:0,sugarFreeStreak:0,exerciseStreak:0,stickers:0,lastDate:null,history:{},achievements:[],inventory:[],doubleXpToday:false,timeLog:{},freedomScore:{},goals:{},reviews:{},financeLog:{}};}
...
export default function App(){
  ...
  var _ft=useState("expense"),finType=_ft[0],setFinType=_ft[1];
  var _fam=useState(""),finAmt=_fam[0],setFinAmt=_fam[1];
  var _fca=useState("food"),finCat=_fca[0],setFinCat=_fca[1];
  var _fno=useState(""),finNote=_fno[0],setFinNote=_fno[1];
  var _fdt=useState(TODAY),finDate=_fdt[0],setFinDate=_fdt[1];
  ...
  var addFinance=useCallback(async function(){if(!curP||!finAmt||parseFloat(finAmt)<=0)return;var cd=data[curP]||getDefault();var fl=Object.assign({},cd.financeLog||{});var entries=(fl[finDate]||[]).slice();entries.push({id:Date.now().toString(),type:finType,cat:finCat,amount:parseFloat(finAmt),note:finNote});fl[finDate]=entries;var up=Object.assign({},cd,{financeLog:fl});var nd=Object.assign({},data);nd[curP]=up;setData(nd);setFinAmt("");setFinNote("");await save(curP,up);},[curP,data,save,finType,finCat,finAmt,finNote,finDate]);
  var deleteFinance=useCallback(async function(eid){if(!curP)return;var cd=data[curP]||getDefault();var fl=Object.assign({},cd.financeLog||{});var entries=(fl[finDate]||[]).filter(function(e){return e.id!==eid;});fl[finDate]=entries;var up=Object.assign({},cd,{financeLog:fl});var nd=Object.assign({},data);nd[curP]=up;setData(nd);await save(curP,up);},[curP,data,save,finDate]);
  ...
  var adTabs=[{id:"today",icon:"📋",l:"Hành vi"},{id:"time",icon:"⏱️",l:"Thời gian"},{id:"finance",icon:"💰",l:"Tài chính"},{id:"goals",icon:"🎯",l:"Mục tiêu"},{id:"review",icon:"📝",l:"Review"},{id:"stats",icon:"📊",l:"Thống kê"}];
  ...
  if(view==="today"){
    ...
    if(isKid){var ukStart=new Date("2026-03-31");var ukEnd=new Date("2026-06-15");var ukShow=new Date("2026-06-20");var ukNow=new Date(TODAY);if(ukNow>=ukStart&&ukNow<=ukShow){var ukAccXp=0;var ukD=new Date(ukStart);while(ukD<=ukEnd&&ukD<=ukNow){var ukDk=ukD.toISOString().split("T")[0];var ukDh=(pl.history&&pl.history[ukDk])?pl.history[ukDk]:{};KIDS_HABITS.forEach(function(h){if(ukDh[h.id])ukAccXp+=h.xp;});ukD=new Date(ukD.getTime()+86400000);}var ukCapDate=ukNow>ukEnd?ukEnd:ukNow;var ukElapsed=Math.round((ukCapDate-ukStart)/86400000)+1;var ukAvg=ukElapsed>0?(ukAccXp/ukElapsed):0;var ukTargetByNow=ukElapsed*25;var ukPct=Math.min(100,(ukAccXp/1750)*100);var ukBarPct=ukTargetByNow>0?Math.min(100,(ukAccXp/ukTargetByNow)*100):0;var ukBarColor=ukAccXp>=ukTargetByNow?"#FFD700":"#FF6B6B";var ukIsPast=ukNow>ukEnd;var ukMsg,ukMsgColor;if(ukIsPast){if(ukAccXp>=1750){ukMsg="🎉 Đạt!";ukMsgColor="#4ECB71";}else{ukMsg="😢 Chưa đạt";ukMsgColor="#FF6B6B";}}else{if(ukAvg>=25){ukMsg="✅ Đang trên đường đến Anh Quốc!";ukMsgColor="#4ECB71";}else if(ukAvg>=20){ukMsg="⚠️ Cần cố gắng thêm!";ukMsgColor="#FFB347";}else{ukMsg="🔴 Nguy hiểm! Cần tăng tốc!";ukMsgColor="#FF6B6B";}}
    content.push(React.createElement("div",{key:"uk",style:{background:"linear-gradient(135deg,#1a237e,#283593)",borderRadius:12,padding:14,marginBottom:8,boxShadow:"0 2px 8px rgba(0,0,0,0.15)"}},React.createElement("div",{style:{textAlign:"center",marginBottom:10}},React.createElement("div",{style:{fontWeight:800,fontSize:16,color:"white"}},"🇬🇧 Chuyến đi Anh Quốc"),React.createElement("div",{style:{fontSize:11,color:"rgba(255,255,255,0.7)",marginTop:2}},"31/3 → 15/6/2026 | Mục tiêu: TB ≥25đ/ngày")),React.createElement("div",{style:{display:"flex",justifyContent:"center",marginBottom:12}},React.createElement("div",{style:{width:120,height:120,borderRadius:"50%",background:"conic-gradient(#FFD700 0% "+ukPct+"%, rgba(255,255,255,0.2) "+ukPct+"% 100%)",position:"relative",display:"flex",alignItems:"center",justifyContent:"center"}},React.createElement("div",{style:{width:80,height:80,borderRadius:"50%",background:"white",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}},React.createElement("div",{style:{fontSize:24,fontWeight:800,color:"#FFD700",lineHeight:1}},ukAccXp),React.createElement("div",{style:{fontSize:10,color:"#888"}},"/1750")))),React.createElement("div",{style:{display:"flex",justifyContent:"space-around",marginBottom:10}},React.createElement("div",{style:{textAlign:"center"}},React.createElement("div",{style:{fontSize:11,color:"rgba(255,255,255,0.7)"}},"📅 Ngày "+ukElapsed+"/77")),React.createElement("div",{style:{textAlign:"center"}},React.createElement("div",{style:{fontSize:11,color:"rgba(255,255,255,0.7)"}},"📊 TB: "+ukAvg.toFixed(1)+"/ngày")),React.createElement("div",{style:{textAlign:"center"}},React.createElement("div",{style:{fontSize:11,color:"rgba(255,255,255,0.7)"}},"🎯 Cần: "+ukTargetByNow))),React.createElement("div",{style:{height:8,borderRadius:4,background:"rgba(255,255,255,0.2)",overflow:"hidden",marginBottom:8}},React.createElement("div",{style:{height:8,borderRadius:4,width:ukBarPct+"%",background:ukBarColor,transition:"width 0.4s"}})),React.createElement("div",{style:{textAlign:"center",fontSize:12,fontWeight:700,color:ukMsgColor}},ukMsg)));}}
  ...
  if(view==="finance"&&!isKid){
    var finEntries=(pl.financeLog&&pl.financeLog[finDate])||[];
    var finTotalIncome=0,finTotalExpense=0;
    finEntries.forEach(function(e){if(e.type==="income")finTotalIncome+=e.amount;else finTotalExpense+=e.amount;});
    var finNet=finTotalIncome-finTotalExpense;
    var finDateObj=new Date(finDate+"T00:00:00");
    var finDayNames=["Chủ nhật","Thứ 2","Thứ 3","Thứ 4","Thứ 5","Thứ 6","Thứ 7"];
    var finDateDisplay=finDayNames[finDateObj.getDay()]+", "+finDateObj.getDate()+"/"+(finDateObj.getMonth()+1)+"/"+finDateObj.getFullYear();
    var finPrevDate=function(){var d=new Date(finDate+"T00:00:00");d.setDate(d.getDate()-1);setFinDate(d.toISOString().split("T")[0]);};
    var finNextDate=function(){var d=new Date(finDate+"T00:00:00");d.setDate(d.getDate()+1);if(d<=new Date())setFinDate(d.toISOString().split("T")[0]);};
    var finIsToday=finDate===TODAY;
    content.push(React.createElement("div",{key:"fin-sum",style:Object.assign({},S.cd,{background:"linear-gradient(135deg,#1a237e,#283593)",color:"white"})},
      React.createElement("div",{style:{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}},
        React.createElement("button",{onClick:finPrevDate,style:{width:32,height:32,borderRadius:8,border:"1px solid rgba(255,255,255,0.3)",background:"rgba(255,255,255,0.1)",color:"white",fontSize:16,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}},"‹"),
        React.createElement("div",{style:{textAlign:"center"}},
          React.createElement("div",{style:{fontSize:16,fontWeight:800}},"💰 Báo cáo tài chính"),
          React.createElement("div",{style:{fontSize:11,color:"rgba(255,255,255,0.7)"}},finDateDisplay),
          !finIsToday?React.createElement("button",{onClick:function(){setFinDate(TODAY);},style:{marginTop:4,padding:"2px 10px",borderRadius:6,border:"1px solid rgba(255,255,255,0.4)",background:"rgba(255,255,255,0.15)",color:"#FFD700",fontSize:10,fontWeight:700,cursor:"pointer"}},"↻ Hôm nay"):null),
        React.createElement("button",{onClick:finNextDate,style:{width:32,height:32,borderRadius:8,border:"1px solid rgba(255,255,255,0.3)",background:finIsToday?"rgba(255,255,255,0.05)":"rgba(255,255,255,0.1)",color:finIsToday?"rgba(255,255,255,0.3)":"white",fontSize:16,cursor:finIsToday?"default":"pointer",display:"flex",alignItems:"center",justifyContent:"center"}},"›")),
      React.createElement("div",{style:{display:"flex",justifyContent:"space-around"}},
        React.createElement("div",{style:{textAlign:"center"}},
          React.createElement("div",{style:{fontSize:10,color:"rgba(255,255,255,0.7)"}},"Thu nhập"),
          React.createElement("div",{style:{fontSize:16,fontWeight:800,color:"#4ECB71"}},"+"+formatVND(finTotalIncome))),
        React.createElement("div",{style:{textAlign:"center"}},
          React.createElement("div",{style:{fontSize:10,color:"rgba(255,255,255,0.7)"}},"Chi tiêu"),
          React.createElement("div",{style:{fontSize:16,fontWeight:800,color:"#FF6B6B"}},"-"+formatVND(finTotalExpense))),
        React.createElement("div",{style:{textAlign:"center"}},
          React.createElement("div",{style:{fontSize:10,color:"rgba(255,255,255,0.7)"}},"Còn lại"),
          React.createElement("div",{style:{fontSize:16,fontWeight:800,color:finNet>=0?"#FFD700":"#FF6B6B"}},(finNet>=0?"+":"")+formatVND(finNet))))));
    var finCatsForType=finType==="income"?FINANCE_INCOME_CATS:FINANCE_EXPENSE_CATS;
    content.push(React.createElement("div",{key:"fin-form",style:S.cd},
      React.createElement("div",{style:{display:"flex",gap:4,marginBottom:8}},
        React.createElement("button",{onClick:function(){setFinType("expense");setFinCat("food");},style:{flex:1,padding:"7px",borderRadius:8,border:finType==="expense"?"2px solid #FF6B6B":"2px solid #EEE",background:finType==="expense"?"#FF6B6B10":"white",color:finType==="expense"?"#FF6B6B":"#999",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"inherit"}},"💸 Chi tiêu"),
        React.createElement("button",{onClick:function(){setFinType("income");setFinCat("salary");},style:{flex:1,padding:"7px",borderRadius:8,border:finType==="income"?"2px solid #4ECB71":"2px solid #EEE",background:finType==="income"?"#4ECB7110":"white",color:finType==="income"?"#4ECB71":"#999",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"inherit"}},"💰 Thu nhập")),
      React.createElement("div",{style:{display:"flex",flexWrap:"wrap",gap:4,marginBottom:8}},
        finCatsForType.map(function(c){return React.createElement("button",{key:c.id,onClick:function(){setFinCat(c.id);},style:{padding:"4px 8px",borderRadius:8,border:finCat===c.id?"2px solid "+c.color:"2px solid #EEE",background:finCat===c.id?c.color+"15":"white",fontSize:11,cursor:"pointer",fontWeight:finCat===c.id?700:400,color:finCat===c.id?c.color:"#666",fontFamily:"inherit"}},c.icon+" "+c.name);})),
      React.createElement("div",{style:{display:"flex",gap:6,marginBottom:6}},
        React.createElement("input",{type:"number",value:finAmt,onChange:function(e){setFinAmt(e.target.value);},placeholder:"Số tiền (VNĐ)",style:{flex:1,padding:"8px 12px",borderRadius:8,border:"2px solid #EEE",fontSize:14,fontWeight:700,outline:"none",fontFamily:"inherit"}}),
        React.createElement("input",{type:"text",value:finNote,onChange:function(e){setFinNote(e.target.value);},placeholder:"Ghi chú...",style:{flex:1,padding:"8px 12px",borderRadius:8,border:"2px solid #EEE",fontSize:13,outline:"none",fontFamily:"inherit"}})),
      React.createElement("button",{onClick:function(){addFinance();},style:{width:"100%",padding:"9px",borderRadius:8,border:"none",background:finType==="income"?"#4ECB71":"#FF6B6B",color:"white",fontWeight:800,fontSize:14,cursor:"pointer",fontFamily:"inherit"}},"+ Thêm "+(finType==="income"?"thu nhập":"chi tiêu"))));
    if(finTotalExpense>0){
      var expByCategory={};
      finEntries.filter(function(e){return e.type==="expense";}).forEach(function(e){expByCategory[e.cat]=(expByCategory[e.cat]||0)+e.amount;});
      var expPieStops=[];var expCumPct=0;var expLegend=[];
      FINANCE_EXPENSE_CATS.forEach(function(c){var v=expByCategory[c.id]||0;if(v>0){var pct=(v/finTotalExpense)*100;expPieStops.push(c.color+" "+expCumPct+"% "+(expCumPct+pct)+"%");expLegend.push({name:c.name,icon:c.icon,color:c.color,amount:v,pct:Math.round(pct)});expCumPct+=pct;}});
      var expGrad="conic-gradient("+expPieStops.join(",")+")";
      content.push(React.createElement("div",{key:"fin-pie",style:S.cd},
        React.createElement("div",{style:{fontWeight:700,fontSize:13,marginBottom:10}},"📊 Phân bổ chi tiêu"),
        React.createElement("div",{style:{display:"flex",justifyContent:"center",marginBottom:12}},
          React.createElement("div",{style:{width:150,height:150,borderRadius:"50%",background:expGrad,position:"relative",boxShadow:"0 2px 12px rgba(0,0,0,0.1)"}},
            React.createElement("div",{style:{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:90,height:90,borderRadius:"50%",background:"white",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}},
              React.createElement("div",{style:{fontSize:16,fontWeight:800,color:"#FF6B6B"}},formatVND(finTotalExpense)),
              React.createElement("div",{style:{fontSize:10,color:"#888"}},"Chi tiêu")))),
        React.createElement("div",{style:{display:"flex",flexWrap:"wrap",gap:6,justifyContent:"center"}},
          expLegend.map(function(it){return React.createElement("div",{key:it.name,style:{display:"flex",alignItems:"center",gap:4,fontSize:11,background:it.color+"15",padding:"3px 8px",borderRadius:8}},
            React.createElement("span",{style:{width:8,height:8,borderRadius:"50%",background:it.color,display:"inline-block",flexShrink:0}}),
            React.createElement("span",null,it.icon+" "+it.name),
            React.createElement("span",{style:{fontWeight:700,color:it.color}},formatVND(it.amount)+" ("+it.pct+"%)"));}))));
    }
    if(finEntries.length>0){
      content.push(React.createElement("div",{key:"fin-list",style:S.cd},
        React.createElement("div",{style:{fontWeight:700,fontSize:13,marginBottom:8}},"📋 Chi tiết hôm nay"),
        finEntries.slice().reverse().map(function(e){
          var catList=e.type==="income"?FINANCE_INCOME_CATS:FINANCE_EXPENSE_CATS;
          var cat=catList.find(function(c){return c.id===e.cat;})||{icon:"📦",name:"Khác",color:"#999"};
          return React.createElement("div",{key:e.id,style:{display:"flex",alignItems:"center",gap:8,padding:"6px 0",borderBottom:"1px solid #F5F5F5"}},
            React.createElement("span",{style:{fontSize:20}},cat.icon),
            React.createElement("div",{style:{flex:1}},
              React.createElement("div",{style:{fontWeight:600,fontSize:12,color:"#333"}},cat.name),
              e.note?React.createElement("div",{style:{fontSize:10,color:"#999"}},e.note):null),
            React.createElement("div",{style:{fontWeight:800,fontSize:13,color:e.type==="income"?"#4ECB71":"#FF6B6B"}},(e.type==="income"?"+":"-")+formatVND(e.amount)),
            React.createElement("button",{onClick:function(){deleteFinance(e.id);},style:{width:22,height:22,borderRadius:6,border:"1px solid #EEE",background:"white",color:"#CCC",fontSize:11,cursor:"pointer",fontFamily:"inherit"}},"✕"));})));
    }
    var finWeekData=[];var finWeekMaxVal=0;
    for(var fi=0;fi<7;fi++){var fd=new Date(Date.now()-(6-fi)*86400000);var fk=fd.toISOString().split("T")[0];var fe=(pl.financeLog&&pl.financeLog[fk])||[];var fInc=0,fExp=0;fe.forEach(function(e){if(e.type==="income")fInc+=e.amount;else fExp+=e.amount;});finWeekMaxVal=Math.max(finWeekMaxVal,fInc,fExp);finWeekData.push({day:dn[fd.getDay()],income:fInc,expense:fExp});}
    if(finWeekMaxVal>0){
      var weekTotalInc=0,weekTotalExp=0;finWeekData.forEach(function(d){weekTotalInc+=d.income;weekTotalExp+=d.expense;});
      content.push(React.createElement("div",{key:"fin-week",style:S.cd},
        React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}},
          React.createElement("div",{style:{fontWeight:700,fontSize:13}},"📆 7 ngày qua"),
          React.createElement("div",{style:{fontSize:11,color:"#888"}},"Thu: "+formatVND(weekTotalInc)+" | Chi: "+formatVND(weekTotalExp))),
        React.createElement("div",{style:{display:"flex",gap:3,justifyContent:"space-between",alignItems:"flex-end",height:90,marginBottom:8}},
          finWeekData.map(function(d,i){
            var incH=finWeekMaxVal>0?Math.max((d.income/finWeekMaxVal)*70,d.income>0?3:0):0;
            var expH=finWeekMaxVal>0?Math.max((d.expense/finWeekMaxVal)*70,d.expense>0?3:0):0;
            return React.createElement("div",{key:i,style:{flex:1,textAlign:"center"}},
              React.createElement("div",{style:{display:"flex",gap:1,justifyContent:"center",alignItems:"flex-end",height:70}},
                React.createElement("div",{style:{width:8,height:incH,borderRadius:3,background:"#4ECB71",transition:"height 0.3s"}}),
                React.createElement("div",{style:{width:8,height:expH,borderRadius:3,background:"#FF6B6B",transition:"height 0.3s"}})),
              React.createElement("div",{style:{fontSize:9,color:"#888",marginTop:3}},d.day));})),
    ...
    var mDaysInMonth=new Date(mYear,mMonth+1,0).getDate();
    var mMonthNames=["Tháng 1","Tháng 2","Tháng 3","Tháng 4","Tháng 5","Tháng 6","Tháng 7","Tháng 8","Tháng 9","Tháng 10","Tháng 11","Tháng 12"];
    var mLabel=mMonthNames[mMonth]+"/"+mYear;
    var mTotalInc=0,mTotalExp=0,mCatExp={},mCatInc={},mDailyArr=[],mDayMax=0,mActiveDays=0;
    for(var mi2=1;mi2<=mDaysInMonth;mi2++){var md2=new Date(mYear,mMonth,mi2);if(md2>mNow)break;var mk2=md2.toISOString().split("T")[0];var me2=(pl.financeLog&&pl.financeLog[mk2])||[];var dInc=0,dExp=0;me2.forEach(function(e){if(e.type==="income"){dInc+=e.amount;mTotalInc+=e.amount;mCatInc[e.cat]=(mCatInc[e.cat]||0)+e.amount;}else{dExp+=e.amount;mTotalExp+=e.amount;mCatExp[e.cat]=(mCatExp[e.cat]||0)+e.amount;}});if(dInc>0||dExp>0)mActiveDays++;mDayMax=Math.max(mDayMax,dInc,dExp);mDailyArr.push({day:mi2,income:dInc,expense:dExp});}
    var mNetTotal=mTotalInc-mTotalExp;var mElapsed=mDailyArr.length;
    var mAvgExp=mElapsed>0?mTotalExp/mElapsed:0;var mAvgInc=mElapsed>0?mTotalInc/mElapsed:0;
    var mProjExp=Math.round(mAvgExp*mDaysInMonth);var mProjInc=Math.round(mAvgInc*mDaysInMonth);
    content.push(React.createElement("div",{key:"fin-month-hdr",style:Object.assign({},S.cd,{background:"linear-gradient(135deg,#0d47a1,#1565c0)",color:"white",marginTop:12})},
      React.createElement("div",{style:{textAlign:"center",marginBottom:10}},
        React.createElement("div",{style:{fontSize:16,fontWeight:800}},"📊 Báo cáo tháng"),
        React.createElement("div",{style:{fontSize:12,color:"rgba(255,255,255,0.7)"}},mLabel+" | "+mElapsed+"/"+mDaysInMonth+" ngày")),
      React.createElement("div",{style:{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:10}},
        React.createElement("div",{style:{textAlign:"center",background:"rgba(255,255,255,0.1)",borderRadius:8,padding:"8px 4px"}},
          React.createElement("div",{style:{fontSize:9,color:"rgba(255,255,255,0.6)"}},"Tổng thu"),
          React.createElement("div",{style:{fontSize:15,fontWeight:800,color:"#4ECB71"}},formatVND(mTotalInc))),
        React.createElement("div",{style:{textAlign:"center",background:"rgba(255,255,255,0.1)",borderRadius:8,padding:"8px 4px"}},
          React.createElement("div",{style:{fontSize:9,color:"rgba(255,255,255,0.6)"}},"Tổng chi"),
          React.createElement("div",{style:{fontSize:15,fontWeight:800,color:"#FF6B6B"}},formatVND(mTotalExp))),
        React.createElement("div",{style:{textAlign:"center",background:"rgba(255,255,255,0.1)",borderRadius:8,padding:"8px 4px"}},
          React.createElement("div",{style:{fontSize:9,color:"rgba(255,255,255,0.6)"}},"Còn lại"),
          React.createElement("div",{style:{fontSize:15,fontWeight:800,color:mNetTotal>=0?"#FFD700":"#FF6B6B"}},(mNetTotal>=0?"+":"")+formatVND(mNetTotal)))),
      React.createElement("div",{style:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}},
        React.createElement("div",{style:{textAlign:"center",background:"rgba(255,255,255,0.08)",borderRadius:8,padding:"6px 4px"}},
          React.createElement("div",{style:{fontSize:9,color:"rgba(255,255,255,0.5)"}},"TB chi/ngày"),
          React.createElement("div",{style:{fontSize:13,fontWeight:700,color:"#FFB347"}},formatVND(Math.round(mAvgExp)))),
        React.createElement("div",{style:{textAlign:"center",background:"rgba(255,255,255,0.08)",borderRadius:8,padding:"6px 4px"}},
          React.createElement("div",{style:{fontSize:9,color:"rgba(255,255,255,0.5)"}},"Dự kiến chi cả tháng"),
          React.createElement("div",{style:{fontSize:13,fontWeight:700,color:"#FFB347"}},"~"+formatVND(mProjExp))))));
    if(mTotalExp>0){
      var mExpSorted=FINANCE_EXPENSE_CATS.filter(function(c){return(mCatExp[c.id]||0)>0;}).sort(function(a,b){return(mCatExp[b.id]||0)-(mCatExp[a.id]||0);});
      var mExpMax=mExpSorted.length>0?(mCatExp[mExpSorted[0].id]||0):1;
      var mPieStops=[];var mCumPct=0;var mPieLegend=[];
      mExpSorted.forEach(function(c){var v=mCatExp[c.id]||0;var pct=(v/mTotalExp)*100;mPieStops.push(c.color+" "+mCumPct+"% "+(mCumPct+pct)+"%");mPieLegend.push({name:c.name,icon:c.icon,color:c.color,amount:v,pct:Math.round(pct)});mCumPct+=pct;});
      var mExpGrad="conic-gradient("+mPieStops.join(",")+")";
      content.push(React.createElement("div",{key:"fin-month-cat",style:S.cd},
        React.createElement("div",{style:{fontWeight:700,fontSize:13,marginBottom:10}},"🏷️ Chi tiêu theo danh mục ("+mLabel+")"),
        React.createElement("div",{style:{display:"flex",justifyContent:"center",marginBottom:12}},
          React.createElement("div",{style:{width:140,height:140,borderRadius:"50%",background:mExpGrad,position:"relative",boxShadow:"0 2px 12px rgba(0,0,0,0.1)"}},
            React.createElement("div",{style:{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:85,height:85,borderRadius:"50%",background:"white",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}},
              ...
              React.createElement("div",{style:{fontSize:9,color:"#888"}},"Tổng chi")))),
        mExpSorted.map(function(c){var v=mCatExp[c.id]||0;var bw=Math.round((v/mExpMax)*100);var pct=Math.round((v/mTotalExp)*100);return React.createElement("div",{key:"mc_"+c.id,style:{display:"flex",alignItems:"center",gap:6,marginBottom:6}},
          ...
          React.createElement("div",{style:{flex:1,minWidth:0}},
            React.createElement("div",{style:{display:"flex",justifyContent:"space-between",marginBottom:2}},
              React.createElement("span",{style:{fontSize:11,fontWeight:600,color:"#333"}},c.name),
              React.createElement("span",{style:{fontSize:11,fontWeight:700,color:c.color}},formatVND(v)+" ("+pct+"%)")),
    ...
    if(mTotalInc>0){
      var mIncSorted=FINANCE_INCOME_CATS.filter(function(c){return(mCatInc[c.id]||0)>0;}).sort(function(a,b){return(mCatInc[b.id]||0)-(mCatInc[a.id]||0);});
      var mIncMax=mIncSorted.length>0?(mCatInc[mIncSorted[0].id]||0):1;
      content.push(React.createElement("div",{key:"fin-month-inc",style:S.cd},
        React.createElement("div",{style:{fontWeight:700,fontSize:13,marginBottom:10}},"💰 Thu nhập theo nguồn ("+mLabel+")"),
        mIncSorted.map(function(c){var v=mCatInc[c.id]||0;var bw=Math.round((v/mIncMax)*100);var pct=Math.round((v/mTotalInc)*100);return React.createElement("div",{key:"mi_"+c.id,style:{display:"flex",alignItems:"center",gap:6,marginBottom:6}},
          React.createElement("span",{style:{fontSize:16,width:22,textAlign:"center",flexShrink:0}},c.icon),
          React.createElement("div",{style:{flex:1,minWidth:0}},
            React.createElement("div",{style:{display:"flex",justifyContent:"space-between",marginBottom:2}},
              React.createElement("span",{style:{fontSize:11,fontWeight:600,color:"#333"}},c.name),
    ...
    if(mDailyArr.length>0&&mDayMax>0){
      content.push(React.createElement("div",{key:"fin-month-trend",style:S.cd},
        React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}},
          React.createElement("div",{style:{fontWeight:700,fontSize:13}},"📈 Xu hướng "+mLabel),
          React.createElement("div",{style:{fontSize:10,color:"#888"}},mActiveDays+" ngày có giao dịch")),
        React.createElement("div",{style:{display:"flex",gap:1,justifyContent:"space-between",alignItems:"flex-end",height:80,marginBottom:6,overflowX:"auto"}},
          mDailyArr.map(function(d,i){
            var incH=mDayMax>0?Math.max((d.income/mDayMax)*65,d.income>0?2:0):0;
            var expH=mDayMax>0?Math.max((d.expense/mDayMax)*65,d.expense>0?2:0):0;
            var isSelected=finDate===mYear+"-"+String(mMonth+1).padStart(2,"0")+"-"+String(d.day).padStart(2,"0");
            return React.createElement("div",{key:i,onClick:function(){var dd=mYear+"-"+String(mMonth+1).padStart(2,"0")+"-"+String(d.day).padStart(2,"0");setFinDate(dd);},style:{flex:1,textAlign:"center",cursor:"pointer",minWidth:10,opacity:isSelected?1:0.8}},
              React.createElement("div",{style:{display:"flex",gap:0,justifyContent:"center",alignItems:"flex-end",height:65}},
                React.createElement("div",{style:{width:4,height:incH,borderRadius:2,background:isSelected?"#2ECC71":"#4ECB71",transition:"height 0.3s"}}),
                React.createElement("div",{style:{width:4,height:expH,borderRadius:2,background:isSelected?"#E74C3C":"#FF6B6B",transition:"height 0.3s"}})),
              d.day%5===1||d.day===mDailyArr.length?React.createElement("div",{style:{fontSize:7,color:isSelected?pr.theme:"#AAA",marginTop:2,fontWeight:isSelected?800:400}},d.day):null);})),
        React.createElement("div",{style:{display:"flex",gap:12,justifyContent:"center",marginTop:4}},
          React.createElement("div",{style:{display:"flex",alignItems:"center",gap:4,fontSize:10}},React.createElement("div",{style:{width:7,height:7,borderRadius:2,background:"#4ECB71"}}),React.createElement("span",null,"Thu")),
          React.createElement("div",{style:{display:"flex",alignItems:"center",gap:4,fontSize:10}},React.createElement("div",{style:{width:7,height:7,borderRadius:2,background:"#FF6B6B"}}),React.createElement("span",null,"Chi")),
          React.createElement("div",{style:{fontSize:10,color:"#888"}},"Nhấn cột để xem ngày"))));
    }
  }
