...
var KIDS_HABITS = [
  ...
  { id: "math_skill", cat: "learn", label: "Phát triển năng lực Toán", emoji: "🔢", xp: 5 },
  { id: "english_skill", cat: "learn", label: "Phát triển năng lực tiếng Anh", emoji: "🇬🇧", xp: 5 },
  { id: "daily_challenge", cat: "leader", label: "Thực hành thử thách", emoji: "🎯", xp: 10 },
];
...
var KIDS_CATS = { sleep: { name: "Giấc ngủ", color: "#6C63FF", icon: "🌙" }, food: { name: "Dinh dưỡng", color: "#FF6B6B", icon: "🍎" }, move: { name: "Vận động", color: "#4ECB71", icon: "⚡" }, mind: { name: "Tinh thần", color: "#FFB347", icon: "✨" }, learn: { name: "Học tập", color: "#3498DB", icon: "📚" }, leader: { name: "Lãnh đạo tương lai", color: "#9B59B6", icon: "👑" } };
...
var PHASES = [
  { id: 1, name: "Khởi động", months: "T3-T4", target: 50, focus: ["sleep"] },
  { id: 2, name: "Tập luyện", months: "T5-T7", target: 50, focus: ["sleep", "food"] },
  { id: 3, name: "Chuyển giao", months: "T8-T10", target: 50, focus: ["sleep", "food", "move"] },
  { id: 4, name: "Tự chủ", months: "T11-T12", target: 50, focus: ["sleep", "food", "move", "mind"] },
];
...
var TREASURES = [
  { id: "ba_mission", name: "Ba thực hiện nhiệm vụ <5 phút", icon: "🦁", stickers: 5 },
  { id: "screen_10min", name: "10 phút sử dụng thiết bị", icon: "📱", stickers: 10 },
  { id: "cash_10k", name: "10.000đ tiền mặt", icon: "💰", stickers: 10 },
  { id: "comic_15min", name: "15 phút đọc truyện tranh", icon: "📖", stickers: 10 },
  { id: "bookstore", name: "Đi nhà sách Tiến Thọ", icon: "📚", stickers: 30 },
  { id: "toy_50k", name: "Đồ chơi dưới 50K", icon: "🧸", stickers: 30 },
  ...
  { id: "star_incorrect", name: "Tích sao chưa chính xác", icon: "⚠️", stickers: 10 },
];
...
export default function App(){
  ...
  var toggleH=useCallback(async function(hid){
    ...
    var cd=data[curP]||getDefault();var st=isKid?50:30;var mx=isKid?43:100;
    ...
    var newStickers = isKid ? Math.floor(tx / 3) : 0; var prevStickers = isKid ? Math.floor(ptx / 3) : 0; var stk = newStickers - prevStickers;
  ...
  var updateTime=useCallback(async function(cid,delta){if(!curP)return;var ne=Object.assign({},timeE);ne[cid]=Math.max(0,(timeE[cid]||0)+delta);setTimeE(ne);var cd=data[curP]||getDefault();var tl=Object.assign({},cd.timeLog);tl[TODAY]=ne;var up=Object.assign({},cd,{timeLog:tl});if(isKid&&delta!==0){var newXp=Math.max(0,(up.totalXp||0)+(delta>0?2:-2));up=Object.assign({},up,{totalXp:newXp,stickers:Math.floor(newXp/3)});}var nd=Object.assign({},data);nd[curP]=up;setData(nd);await save(curP,up);},[curP,timeE,data,save,isKid]);
  ...
  var st=isKid?50:30,mx=isKid?43:100;var tx=0;habits.forEach(function(h){if(checked[h.id])tx+=h.xp;});
  ...
  if(view==="today"){
    ...
    if(isKid){var ukStart=new Date("2026-06-01");var ukEnd=new Date("2026-08-15");var ukShow=new Date("2026-08-20");var ukNow=new Date(TODAY);if(ukNow>=ukStart&&ukNow<=ukShow){var ukAccXp=0;var ukD=new Date(ukStart);while(ukD<=ukEnd&&ukD<=ukNow){var ukDk=ukD.toISOString().split("T")[0];var ukDh=(pl.history&&pl.history[ukDk])?pl.history[ukDk]:{};KIDS_HABITS.forEach(function(h){if(ukDh[h.id])ukAccXp+=h.xp;});ukD=new Date(ukD.getTime()+86400000);}var ukCapDate=ukNow>ukEnd?ukEnd:ukNow;var ukElapsed=Math.round((ukCapDate-ukStart)/86400000)+1;var ukAvg=ukElapsed>0?(ukAccXp/ukElapsed):0;var ukTargetByNow=ukElapsed*25;var ukPct=Math.min(100,(ukAccXp/3500)*100);var ukBarPct=ukTargetByNow>0?Math.min(100,(ukAccXp/ukTargetByNow)*100):0;var ukBarColor=ukAccXp>=ukTargetByNow?"#FFD700":"#FF6B6B";var ukIsPast=ukNow>ukEnd;var ukMsg,ukMsgColor;if(ukIsPast){if(ukAccXp>=1750){ukMsg="🎉 Đạt!";ukMsgColor="#4ECB71";}else{ukMsg="😢 Chưa đạt";ukMsgColor="#FF6B6B";}}else{if(ukAvg>=25){ukMsg="✅ Đang trên đường đến Hồng Kông!";ukMsgColor="#4ECB71";}else if(ukAvg>=20){ukMsg="⚠️ Cần cố gắng thêm!";ukMsgColor="#FFB347";}else{ukMsg="🔴 Nguy hiểm! Cần tăng tốc!";ukMsgColor="#FF6B6B";}}
    content.push(React.createElement("div",{key:"uk",style:{background:"linear-gradient(135deg,#C8102E,#FF4500)",borderRadius:12,padding:14,marginBottom:8,boxShadow:"0 2px 8px rgba(0,0,0,0.15)"}},React.createElement("div",{style:{textAlign:"center",marginBottom:10}},React.createElement("div",{style:{fontWeight:800,fontSize:16,color:"white"}},"🇭🇰 Chuyến đi Hồng Kông"),React.createElement("div",{style:{fontSize:11,color:"rgba(255,255,255,0.7)",marginTop:2}},"1/6 → 15/8/2026 | Mục tiêu: TB ≥25đ/ngày")),React.createElement("div",{style:{display:"flex",justifyContent:"center",marginBottom:12}},React.createElement("div",{style:{width:120,height:120,borderRadius:"50%",background:"conic-gradient(#FFD700 0% "+ukPct+"%, rgba(255,255,255,0.2) "+ukPct+"% 100%)",position:"relative",display:"flex",alignItems:"center",justifyContent:"center"}},React.createElement("div",{style:{width:80,height:80,borderRadius:"50%",background:"white",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}},React.createElement("div",{style:{fontSize:24,fontWeight:800,color:"#FFD700",lineHeight:1}},ukAccXp),React.createElement("div",{style:{fontSize:10,color:"#888"}},"/3500")))),React.createElement("div",{style:{display:"flex",justifyContent:"space-around",marginBottom:10}},React.createElement("div",{style:{textAlign:"center"}},React.createElement("div",{style:{fontSize:11,color:"rgba(255,255,255,0.7)"}},"📅 Ngày "+ukElapsed+"/76")),React.createElement("div",{style:{textAlign:"center"}},React.createElement("div",{style:{fontSize:11,color:"rgba(255,255,255,0.7)"}},"📊 TB: "+ukAvg.toFixed(1)+"/ngày")),React.createElement("div",{style:{textAlign:"center"}},React.createElement("div",{style:{fontSize:11,color:"rgba(255,255,255,0.7)"}},"🎯 Cần: "+ukTargetByNow))),React.createElement("div",{style:{height:8,borderRadius:4,background:"rgba(255,255,255,0.2)",overflow:"hidden",marginBottom:8}},React.createElement("div",{style:{height:8,borderRadius:4,width:ukBarPct+"%",background:ukBarColor,transition:"width 0.4s"}})),React.createElement("div",{style:{textAlign:"center",fontSize:12,fontWeight:700,color:ukMsgColor}},ukMsg)));}}
